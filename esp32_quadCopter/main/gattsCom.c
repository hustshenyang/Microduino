...
char bt_name[15];
uint16_t bleData[8] = {1500, 1500, 1500, 1500, 1000, 1000, 1000, 1000};
...
#define TEST_DEVICE_NAME            "mCookie%X%X"
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        sprintf(bt_name, TEST_DEVICE_NAME, bt_mac[4], bt_mac[5]);
    ...
    case ESP_GATTS_READ_EVT: {
        ESP_LOGI(GATTS_TAG, "GATT_READ_EVT, conn_id %d, trans_id %d, handle %d, gatts_if %d\n", param->read.conn_id, param->read.trans_id, param->read.handle, gatts_if);
        esp_gatt_rsp_t rsp;
...
//        ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, conn_id %d, trans_id %d, handle %d, gatts_if %d\n", param->write.conn_id, param->write.trans_id, param->write.handle, gatts_if);
//        ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, value len %d, value %32x\n", param->write.len, *(uint32_t *)param->write.value);
        esp_ble_gatts_send_response(gatts_if, param->write.conn_id, param->write.trans_id, ESP_GATT_OK, NULL);
        if(param->write.len == 16){
        	memcpy((uint8_t *)bleData, param->write.value, 16);
