...
{
	I2C_writeByte(FMTISensorAdd_I2C,0xf4, 0x2e);
}

//寮�濮嬫皵鍘嬭浆鎹�
void start_pressure(void)
{
	I2C_writeByte(FMTISensorAdd_I2C,0xf4, 0xf4);
}

uint32_t Read_data(void)
{
	uint8_t buf[3];
	I2C_readBytes(FMTISensorAdd_I2C,0xf6, 3, buf);
...
void read_offset(void)
{
	...
	for(uint8_t i = 0;i < 9;i++) {
		I2C_readBytes(FMTISensorAdd_I2C,(0xaa+i*2), 2, buf);
		R[i] = ((uint8_t) buf[0] << 8) | buf[1];
	}
	I2C_readByte(FMTISensorAdd_I2C,0xa4,buf);
	I2C_readByte(FMTISensorAdd_I2C,0xf1,buf+1);
...
{
	uint8_t whoami = 0x00;
	I2C_readByte(FMTISensorAdd_I2C,0x6b,&whoami);
	if(whoami == 0x42) {
		read_offset();
		Fbm320CaliInit();
		return true;
...
void Fbm320Process(void)
{
	...
	fbm320.UP = Read_data();
	calculate_real_pressure(fbm320.UP, fbm320.UT);
	baroPressureSum = recalculateBarometerTotal(SAMPLE_COUNT_MAX, baroPressureSum, fbm320.RP);
	if(!fbm320.calibrate_finished)
	{
		baroCaliCnt++;
		if(baroCaliCnt > SAMPLE_COUNT_MAX-1)
		{
			fbm320.Reff_P = baroPressureSum/(SAMPLE_COUNT_MAX-1);
			fbm320.calibrate_finished = true;
		}
		return;
	}
	alt = Rel_Altitude(baroPressureSum/(SAMPLE_COUNT_MAX-1),fbm320.Reff_P);//unit:cm
	fbm320.Altitude = SCALE*alt + (1-SCALE)*fbm320.Altitude;
}
...
{
	baroCaliCnt = 0;
	fbm320.calibrate_finished = false;
}
